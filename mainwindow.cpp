...
{
    PdfDocument pdf(Poppler::Document::load(filename).release());
...
{
    int page_count = 0;
    // --- 修正箇所：呼び出し元が「ファイル1」か「2」かを判定して代入する ---
    if (filename == filename1LineEdit->text()) {
        pdf1 = getPdf(filename);
    } else {
        pdf2 = getPdf(filename);
    }
    // 以降、ローカルの pdf ではなくメンバ変数を使う（あるいは pdf に代入する）
    PdfDocument pdf = (filename == filename1LineEdit->text()) ? pdf1 : pdf2;
...
void MainWindow::compare()
{
    ...
    const QPair<int, int> pair = comparePages(filename1, pdf1, filename2,
                                              pdf2);
    compareUpdateUi(pair, timer.elapsed());
}
                                                    ...
                                                    PdfPage page2)
{
    ...
    for (int i = 0; i < list1.count(); ++i) {
        // 1. QSharedPointer が中身を持っているかチェック
        if (list1[i].isNull() || list2[i].isNull()) continue;
        // 2. Poppler::TextBox 自体が壊れていないかチェック
        auto* box1 = list1[i].data();
        auto* box2 = list2[i].data();
        if (box1 && box2) {
            // 2. QSharedPointer 経由で text() を呼ぶ
...
//refactor prima di lanciare
    if (currentCompareIndex == CompareAppearance) {
        ...
        if (image1.isNull() || image2.isNull()) return VisualDifference;
        if (image1 != image2)
