...
const TextBoxList getTextBoxes(PdfPage page, const QRectF &rect)
{
    ...
    // C++11 の範囲ベース for ループを使用します
    // textList() は QList<std::unique_ptr<Poppler::TextBox>> を返します
    auto list = page->textList();
    for (auto &box : list) {
        if (!box) continue;
        // 以前のコードが Poppler::TextBox* を期待しているため、.get() でポインタを取り出します
        Poppler::TextBox *box_ptr = box.release();

        // --- ここから下は元の「box」を「b」に置き換えて継続 ---
        if (rect.isNull() || rect.contains(box_ptr->boundingBox())){
            // 所有権を release したポインタを QSharedPointer に渡して管理を任せる
            textBoxes.append(QSharedPointer<Poppler::TextBox>(box_ptr));
        } else {
            // リストに入れない場合は、release してしまったので手動で消す必要があります
            delete box_ptr;
        }
    }
    return textBoxes;
