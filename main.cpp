...
int main(int argc, char *argv[])
{
    ...
    // 1. スタイルを「Fusion」に設定（これがダークモードの基盤になります）
    QApplication::setStyle(QStyleFactory::create("Fusion"));

    // 2. ダークモード用の配色（パレット）を作成
    QPalette darkPalette;
    QColor windowColor(60, 60, 60);
    QColor baseColor(45, 45, 45);
    QColor buttonColor(70, 70, 70);
    QColor highlightColor(0, 120, 215); // Windows標準に近いブルー
    ...
    qRegisterMetaType<PagePair>("PagePair");
    ...
    if(!startupParameters.isBatch()) {
        ...
        // 戻り値を bool 型の変数で受け取るか、(void)でキャストして明示的に無視します
        if (!qtTranslator.load("qt_" + language,
                               QLibraryInfo::path(QLibraryInfo::TranslationsPath))) {
            // 読み込めなくても致命的ではないので、デバッグ出力程度に留めるのが一般的です
            qDebug() << "Could not load Qt translation for" << language;
        }
        app.installTranslator(&qtTranslator);
        QTranslator appTranslator;
        if (!appTranslator.load("confrontapdf_" + language, ":/")) {
            // 失敗しても動作には支障がないため、何もしないかデバッグ出力を入れる程度でOKです
        }
